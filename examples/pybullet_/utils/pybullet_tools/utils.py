...
def load_pybullet(filename, fixed_base=True, scale=1., **kwargs):
...
def connect(use_gui=True, shadows=True):
    ...
    # make sure to compile pybullet_ with PYBULLET_USE_NUMPY enabled
...
def get_dynamics_info(body, link=BASE_LINK):
    tmp = p.getDynamicsInfo(body, link, physicsClientId=CLIENT)
...
def create_shape(geometry, pose=unit_pose(), color=(1, 0, 0, 1), specular=None):
    ...
    if 'length' in collision_args:
        # TODO: pybullet_ bug visual => length, collision => height
...
def visual_shape_from_data(data, client=None):
    ...
    return p.createVisualShape(shapeType=data.visualGeometryType,
                               ...
                               length=get_data_height(data), # TODO: pybullet_ bug
...
def approximate_as_prism(body, body_pose=unit_pose()): #, **kwargs):
    ...
    """
    Read object vertices in object FOR.
    Transform vertices to camera FOR by compounding object body pose.
    Obtain points aabb bounding box.
    Approximate the entire object as AABB bounding box.
    Return AABB center and extent.
    """
...
def pairwise_link_collision(body1, link1, body2, link2=BASE_LINK, max_distance=MAX_DISTANCE): # 0.0
...
def get_self_link_pairs(body, joints, disabled_collisions=set(), only_moving=True):
    """
    Check collision between pairs of links within the robotic body.
    """
    moving_links = get_moving_links(body, joints)  # moving links set.
    fixed_links = list(set(get_links(body)) - set(moving_links))  # fixed links set.
    check_link_pairs = list(product(moving_links, fixed_links))  # initialize check_link_pairs by moving X fixed links.
    if only_moving:
...
class Attachment(object):
    def __init__(self, parent, parent_link, grasp_pose, child):
        """
        Attach the grasped object - child to the parent object.
        :param parent: Parent object id.
        :param parent_link: Parent object end-effector link.
        :param grasp_pose: The pose to grasp.
        :param child: Child object id.
        """
        self.parent = parent
    ...
    def assign(self):
        """
        Get parent end-effector link pose.
        Get child pose by compose the relative tool pose given by self.grasp_pose with the parent_link_pose.
        Reset child pose by calling PyBullet.
        """
...
# TODO: factor out things that don't depend on pybullet_
