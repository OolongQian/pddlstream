...
from .utils import get_pose, set_pose, get_movable_joints, set_joint_positions, add_fixed_constraint, enable_real_time, \
	disable_real_time, joint_controller, enable_gravity, user_input, wait_for_duration, link_from_name, get_body_name, \
	sample_placement, end_effector_from_body, approach_from_grasp, plan_joint_motion, GraspInfo, Pose, INF, Point, \
	inverse_kinematics, pairwise_collision, remove_fixed_constraint, Attachment, get_sample_fn, step_simulation, \
	refine_path, plan_direct_joint_motion, get_joint_positions
...
TOOL_FRAMES = {'iiwa14': 'iiwa_link_ee_kuka',  # iiwa_link_ee
               }
...
class BodyGrasp(object):
	def __init__(self, body, grasp_pose, approach_pose, robot, link):
		"""
		:param body: Object id to grasp.
		:param grasp_pose: A 6-D coordinate.
		:param approach_pose: A 6-D coordinate.
		:param robot: Robot id.
		:param link: Tool link id (what is tool link? Maybe the end-effector).
		"""
		self.body = body
	...
	def attachment(self):
		"""
		Create and return a new attachment object related to this grasp.
		"""
		return Attachment(self.robot, self.link, self.grasp_pose, self.body)
	
	def assign(self):
		"""
		Create the related attachment object and assign to the robot's end-effector.
		"""
...
class BodyPath(object):
	def __init__(self, body, path, joints=None, attachments=[]):
		"""
		:param body: Body id to move.
		:param path: List of joint positions.
		:param joints: Body joints relevant in movement.
		:param attachments: Related to tool use, but the input data type is unknown.
								The temporary type of attachment is class BodyGrasp.
		"""
		if joints is None:
			joints = get_movable_joints(body)
		self.body = body
	...
	def iterator(self):
		...
		for i, configuration in enumerate(self.path):
			"Invoke p.setJointState directly."
...
def get_grasp_gen(robot, grasp_name):
	...
	def gen(body):
		...
		for grasp_pose in grasp_poses:
			body_grasp = BodyGrasp(body, grasp_pose, grasp_info.approach_pose, robot, end_effector_link)
			yield (body_grasp,)
...
def get_stable_gen(fixed=[]):  # TODO: continuous set of grasps
	def gen(body, surface):
		while True:
			...
			yield (body_pose,)  # TODO: check collisions
...
def get_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
	...
	def fn(body, pose, grasp):
		...
		for i in range(num_attempts):
			"Random sample robot configuration."
			...
			if teleport:
				...
				if path is None:
					...
					continue
			command = Command([BodyPath(robot, path), Attach(body, robot, grasp.link),
			                   BodyPath(robot, path[::-1], attachments=[grasp])])
			return (conf, command)  # TODO: holding collisions
		return None
...
def get_holding_motion_gen(robot, fixed=[], teleport=False, self_collisions=True):
	def fn(conf1, conf2, body, grasp, fluents=[]):
		...
		if teleport:
			...
			obstacles = fixed + assign_fluent_state(fluents)
			path = plan_joint_motion(robot, conf2.joints, conf2.configuration, obstacles=obstacles,
			                         attachments=[grasp.attachment()], self_collisions=self_collisions)
			if path is None:
