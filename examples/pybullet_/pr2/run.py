...
from examples.pybullet_.utils.pybullet_tools.pr2_primitives import Pose, Conf, get_ik_ir_gen, get_motion_gen, \
	get_stable_gen, get_grasp_gen, Attach, Detach, Clean, Cook, control_commands, get_gripper_joints, GripperCommand, \
	apply_commands, State
from examples.pybullet_.utils.pybullet_tools.pr2_problems import cooking_problem, stacking_problem, holding_problem
from examples.pybullet_.utils.pybullet_tools.pr2_utils import get_arm_joints, ARM_NAMES, get_group_joints, \
	get_group_conf
from examples.pybullet_.utils.pybullet_tools.utils import connect, get_pose, is_placement, point_from_pose, disconnect, \
	user_input, get_joint_positions, save_state, restore_state, HideOutput, get_distance, LockRenderer, get_max_limit
...
def place_movable(certified):
	for literal in certified:
		...
		if fact[0] == 'trajgraspcollision':
			_, a, o, g = fact[1:]  # TODO: finish this
...
def pddlstream_from_problem(problem, teleport=False):
	...
	init = [('CanMove',), ('BConf', initial_bq), ('AtBConf', initial_bq), Equal(('PickCost',), 1),
		       Equal(('PlaceCost',), 1), ] + [('Sink', s) for s in problem.sinks] + [('Stove', s) for s in
	                                                                                 problem.stoves] + [
		       ('Connected', b, d) for b, d in problem.buttons] + [('Button', b) for b, _ in problem.buttons]
	...
	goal += [('Holding', a, b) for a, b in problem.goal_holding] + [('On', b, s) for b, s in problem.goal_on] + [
		('Cleaned', b) for b in problem.goal_cleaned] + [('Cooked', b) for b in problem.goal_cooked]
	
	stream_map = {'sample-pose': from_gen_fn(get_stable_gen(problem)),
		'sample-grasp': from_list_fn(get_grasp_gen(problem)),
		'inverse-kinematics': from_gen_fn(get_ik_ir_gen(problem, teleport=teleport)),
		'plan-base-motion': from_fn(get_motion_gen(problem, teleport=teleport)), 'MoveCost': move_cost_fn,
		'TrajPoseCollision': fn_from_constant(False), 'TrajArmCollision': fn_from_constant(False),
...
def main(display=True, teleport=False, partial=False):
	...
	stream_info = {'sample-pose': StreamInfo(PartialInputs('?r')),
		'inverse-kinematics': StreamInfo(PartialInputs('?p')),
		'plan-base-motion': StreamInfo(PartialInputs('?q1 ?q2'), defer=True),
		'MoveCost': FunctionInfo(opt_move_cost_fn), } if partial else {'sample-pose': StreamInfo(from_fn(opt_pose_fn)),
		'inverse-kinematics': StreamInfo(from_fn(opt_ik_fn)), 'plan-base-motion': StreamInfo(from_fn(opt_motion_fn)),
	...
	user_input('Finish?')
	disconnect()  # TODO: need to wrap circular joints
